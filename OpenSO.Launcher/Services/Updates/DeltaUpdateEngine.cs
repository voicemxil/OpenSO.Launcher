...
public sealed class DeltaUpdateEngine
{
    ...
    private static void TryDeleteDir(string path)
    {
        try
        {
            ...
            // A backed-up copy of a read-only file keeps its ReadOnly attribute, and on Windows
            // Directory.Delete refuses to remove read-only entries — clear attributes first so the
            // backup store is always cleaned up after a rollback.
    ...
    /// the patcher isn't running; the delta engine never stages into PatchFiles itself, so anything
    /// there is legacy update.exe state.
    /// </summary>
