...
/// <summary>
/// One entry of the server's update feed (GET {api}/userapi/update). Verbatim port of the game's
/// FSO.Server.Clients.ApiUpdate — snake_case property names match the JSON wire format directly.
/// </summary>
public sealed class ApiUpdate
{
...
/// <summary>
/// The chain of updates to apply to get from one client version to another. Verbatim port of the
/// game's FSO.Server.Clients.UpdatePath so the launcher computes exactly the same route the
/// in-client updater would (delta chain when the installed version is on the feed, otherwise a
/// full-zip start plus any later deltas).
/// </summary>
